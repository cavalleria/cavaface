...
class EvalMegaFace(object):
    ...
    def _get_output_dir(self):
        ...
        self._face_scrub_emb_clean_outdir = os.path.join(
            self._emb_clean_outdir_root, "facescrub"
        )
        ...
        self._face_gallery_emb_clean_outdir = os.path.join(
            self._emb_clean_outdir_root, "megaface"
        )
    ...
    def _infer_and_write(self, imgs_list, net):
        return net.infer_embedding(
            imgs_list,
            read_func=_read_image,
            write_func=_write_bin,
            is_flip=self._args.flip,
        )

    #

    def _extract_embedding_base(
        ...
        print(
            "Finish getting embedding for faces, total: %d, success: %d" % (num, succ)
        )
        return ret
    ...
    def extract_embedding(self, net):
        ...
        ret = self._extract_embedding_base(
            self._face_scrub, net, self._face_scrub_emb_outdir, self._face_scrub_root
        )
        if not ret:
            return False
    ...
    def remove_noisy_embedding(self):
        ...
        for line in self._face_scrub:
            ...
            feature_path = os.path.join(
                self._face_scrub_emb_outdir, a, "%s_%s.bin" % (b, self._algo)
            )
            ...
            feature_path_out = os.path.join(
                feature_dir_out, "%s_%s.bin" % (b, self._algo)
            )
            if not b in self._face_scrub_noisy_dict:
                ...
                if not a in fname2center:
                    fname2center[a] = np.zeros(
                        (feat_dim + feature_ext,), dtype=np.float32
                    )
        ...
        for k in noises:
            ...
            f /= _norm
            feature_path_out = os.path.join(
                self._face_scrub_emb_clean_outdir, a, "%s_%s.bin" % (b, self._algo)
            )
        ...
        for line in self._face_gallery:
            ...
            a1, a2, b = _path[-3], _path[-2], _path[-1]
            feature_path = os.path.join(
                self._face_gallery_emb_outdir, a1, a2, "%s_%s.bin" % (b, self._algo)
            )
        ...
        #
        print(
            "Total number of removing noisy faces in gallery: %d/%d" % (nrof_noises, i)
        )
    ...
    def metric(self):
        ...
        print(
            "LD_LIBRARY_PATH=%s/lib:${LD_LIBRARY_PATH} %s/bin/python -u run_experiment.py %s %s _%s.bin %s -s 1000000 -p %s"
            % (
    ...
    def _cal_and_save_badcase(self):
        ...
        for cid, iprob in dict_prob.items():
            ...
            print(
                "Begin to calculate badcase for class: ", cid, ", with samples: ", inum
            )
            ...
            for igall in range(inum):
                for jprob in range(inum):
                    ...
                    probs_scores.append(
                        [prob_set[jprob][1], prob_set[igall][1], ij_score]
                    )
                #
            #
            ...
            if badcase.shape[0] > 0:
                badcase_score = np.array(scores)[
                    badcase_ind, search_res[badcase_ind]
                ].reshape((badcase.shape[0], -1))
            ...
            #
            print(
                "Finished class with id: ",
                ip_cnt,
                " similarity matrix with shape: ",
                scores_all.shape,
                ", search times: %d" % (len(scores)),
            )
            print("")
            ip_cnt += 1
            sys.stdout.flush()
    ...
    def parse_results_into_file(self):
        result_json_file = os.path.join(
            self._outdir_root, "cmc_facescrub_megaface_%s_1000000_1.json" % (self._algo)
        )
