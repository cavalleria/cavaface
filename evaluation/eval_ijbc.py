...
class EvalIJBC(object):
    ...
    def _get_output_dir(self):
        ...
        self._outdir_root = os.path.join(
            "/".join(self._args.model_path.strip().split("/")[:-1]), "ijbc_%d" % (epoch)
        )
    ...
    def image2template_feature(self, img_feats=None, templates=None, medias=None):
        ...
        template_norm_feats = template_feats / np.sqrt(
            np.sum(template_feats ** 2, -1, keepdims=True) + 0.000001
        )
    ...
    def extract_embedding(self, net):
        ...
        return self._extract_embedding_base(
            self._image_list_path, net, self._emb_outdir_root, self._image_path
        )
    ...
    def _read_templates(self, file_list):
        ...
        for idx, line in enumerate(file_list):
            ...
            feat_name = os.path.join(self._emb_outdir_root, image_path + ".bin")
    ...
    def metric(self):
        ...
        self._read_embedding_base(
            self._image_list_path, self._emb_outdir_root, self._image_path
        )
        ...
        # use dectector score
        img_feats = img_feats * np.matlib.repmat(
            faceness_scores[:, np.newaxis], 1, img_feats.shape[1]
        )

        templates, medias = self.read_template_media_list(self._media_list)
        p1, p2, label = self.read_template_pair_list(self._pair_list)
        template_norm_feats, unique_templates = self.image2template_feature(
