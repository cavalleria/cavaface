...
def count_model_params(model):
    total_params = np.sum(
        [param.numel() if param.requires_grad else 0 for param in model.parameters()]
    )
...
def count_model_flops(model, input_res=[112, 112], multiply_adds=True):
    ...
    def conv_hook(self, input, output):
        ...
        flops = (
            (kernel_ops * (2 if multiply_adds else 1) + bias_ops)
            * output_channels
            * output_height
            * output_width
            * batch_size
    ...
    def foo(net):
        ...
        if not childrens:
            if isinstance(net, torch.nn.Conv2d) or isinstance(
